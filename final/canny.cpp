...
Canny::Canny(string _filename)
    : sigma(1), scale(1), th(60), tl(20), filename(_filename.substr(4)) {
...
Canny::Canny(const CImg<unsigned char> &src)
    : sigma(1), scale(1), th(60), tl(20), img(src) {}
...
CImg<double> Canny::nms(CImg<double> gradx, CImg<double> grady) {
    printf(" BEGIN NMS\n");
...
CImg<double> Canny::doubleThreshold() {
    th = min(th, 255);
    tl = min(tl, 255);
...
CImg<double> Canny::deleteLine(int threshold) {
